...
int Sim7070GDevice::start()
{
  ...
  shortId[5] = '\0';
  snprintf(_pubTopic, sizeof(_pubTopic), "xtr/nodes/%s", shortId);
  snprintf(_subTopic, sizeof(_subTopic), "xtr/server/%s", shortId);
...
int Sim7070GDevice::timeout()
{
  ...
  case MODEM_POWER_ON:
  {
    ...
    return 10000; // Wait for modem to boot
  }
  ...
  case MODEM_WAITING_SUCCESS:
  {
    ...
    return 200; // Poll again after 200 ms
  }
...
static const __FlashStringHelper *modemStateName(ModemState s)
{
  ...
  case MODEM_WAIT_FOR_IP_FIRST:
    return F("MODEM_WAIT_FOR_IP_FIRST");
  case MODEM_CHECK_IP_FIRST:
    return F("MODEM_CHECK_IP_FIRST");
  case MODEM_APPLY_BOOT_CONFIG:
    return F("MODEM_APPLY_BOOT_CONFIG");
  case MODEM_CHECK_SIM_AFTER_CONFIG:
    return F("MODEM_CHECK_SIM_AFTER_CONFIG");
  case MODEM_CHECK_SIGNAL_AFTER_CONFIG:
    return F("MODEM_CHECK_SIGNAL_AFTER_CONFIG");
  case MODEM_SET_APN:
    return F("MODEM_SET_APN");
  case MODEM_WAIT_FOR_IP_AFTER_APN:
    return F("MODEM_WAIT_FOR_IP_AFTER_APN");
  case MODEM_CHECK_IP_AFTER_APN:
    return F("MODEM_CHECK_IP_AFTER_APN");
  case MODEM_SYNC_NTP_TIME:
    return F("MODEM_SYNC_NTP_TIME");
  case MODEM_MQTT_CONNECTING_HANDSHAKE:
    return F("MODEM_MQTT_CONNECTING_HANDSHAKE");
  case MODEM_MQTT_CONNECTING:
    return F("MODEM_MQTT_CONNECTING");
  case MODEM_MQTT_CONNECTED_FIRST:
    return F("MODEM_MQTT_CONNECTED_FIRST");
  case MODEM_MQTT_CONNECTED:
